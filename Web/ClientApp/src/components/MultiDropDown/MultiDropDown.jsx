...
class MultiDropDown extends React.Component {
  ...
  componentWillMount() {
    // check if toggleable
    if (this.props.hideHeader || this.props.hideList) {
      this.toggleable = false;
    } else {
      if (this.props.toggleable) {
        this.toggleable = this.props.toggleable;
      } else this.toggleable = true;
    }
    //check if there is search input
    if (
      typeof this.props.search == "function" ||
      typeof this.props.getListAsync == "function"
    ) {
  ...
  headerKeyDownHandler(e, element) {
    switch (e.keyCode) {
      case 13: //enter
        if (e.target.classList.contains("unselectButton")) {
          this.unselect(e, element);
        }
        if (e.target.classList.contains("drop-down-header")) {
          this.toggle();
        }
        break;
      case 27: //ESC
  ...
  createList(list) {
    ...
    if (!this.props.expandBy) {
      if (this.props.multiSelect) {
        // one level multiselect
        this.props.defaultValue.forEach((value) => {
          headerList.push({
            parentElement: list.find(
              (el) => value === el[this.props.keyProperty],
            ),
          });
        });
      } else {
        // one level 1 value
        let tempElement = list.find(
          (element) =>
            this.props.defaultValue === element[this.props.keyProperty],
        );
        if (tempElement) {
      ...
      if (
        ...
        list.forEach((element) => {
          ...
          element[this.props.expandBy].forEach((innerElement) => {
            let checked = this.props.defaultValue.indexOf(
              innerElement[this.props.expandedKeyProperty],
            );
            if (checked < 0) {
              theyAllAreChecked = false;
            } else {
              tempHeaderList.push({
        ...
        let childElement;
        let findChild = (list) => {
          childElement = list[this.props.expandBy].find(
            (element) =>
              element[this.props.expandedKeyProperty] ===
  ...
  unselect = (e, element) => {
    ...
    if (element.childElement) {
      innerIndex = element.parentElement[this.props.expandBy].indexOf(
  ...
  checkBoxChange = (index, innerIndex) => {
    ...
    if (this.props.expandedMultiSelect) {
      if (innerIndex < 0) {
        ...
        if (allAreSelected || someAreSelected) {
          //delete all inner elements
          element[this.props.expandBy].forEach(
            (element) =>
              (value = this.arrayRemove(
                value,
                element[this.props.expandedKeyProperty],
              )),
          );
        } else {
          //add all inner elements
          element[this.props.expandBy].forEach((element) =>
            value.push(element[this.props.expandedKeyProperty]),
          );
        }
      } else {
        // 2nd level check box was checked
      ...
      if (
        this.props.defaultValue.indexOf(element[this.props.keyProperty]) > -1
      ) {
  ...
  returnFocusToParent() {
    if (this.toggleable) {
      ...
      // return focus to WRAPPER ELEMENT
            ...
            if (innerIndex > -1) {
              ...
              if (!this.props.expandedMultiSelect) {
                this.onDropDownValueChange(
                  list[index][this.props.expandBy][innerIndex][
                    this.props.expandedKeyProperty
                  ],
                );
                this.toggle();
              }
            ...
            if (
              ...
              this.setState({
                ...
                setFocusToInnerIndex:
                  list[index - 1][this.props.expandBy].length - 1,
              });
            } else {
  ...
  onSearchInputValueChange(value) {
    clearTimeout(this.timeoutVar);
    if (
      this.props.searchMinCharacterCount > 1 &&
      value.length < this.props.searchMinCharacterCount
    ) {
  ...
  render() {
    const setStyle = () => {
      ...
      if (
        /*(this.props.multiSelect || this.props.expandedMultiSelect) &&*/ this
          .thereIsSearchInput
      ) {
        style["cursor"] = "text";
      }
    ...
    return (
      <div
        className={
          "drop-down" +
          (this.props.disabled === true ? " drop-down-disabled" : "") +
      ...
      >
        {!this.props.hideHeader && (
          <div
            ...
            onFocus={() => {
              this.dropDownHeaderRef.classList.add("ddhOnFocus");
            }}
            onBlur={() => {
              this.dropDownHeaderRef.classList.remove("ddhOnFocus");
            }}
          >
              ...
              >
                {list.length > 0 &&
                  ...
                  list.map((element, index) => {
                    let targetElement;
                    if (element.childElement !== undefined) {
                      targetElement = element.childElement;
                    } else {
                      ...
                      <li key={index} className="flex-nowrap">
                        {targetElement.color && (
                          <span
                            ...
                            style={{
                              backgroundColor: targetElement.color,
                              marginRight: "0.5rem",
                            }}
                          ></span>
                        )}
                        ...
                        <span style={{ flex: "1 0 auto" }}>
                          {element.childElement
                            ? element.childElement[
                                this.props.expandedTextProperty
                              ] +
                              ", " +
                        ...
                        {(this.props.multiSelect ||
                          ...
                          <button
                            ...
                            onKeyDown={(e) =>
                              this.headerKeyDownHandler(e, element)
                            }
                          >
                            <CloseSVG />
                          </button>
                ...
                {this.thereIsSearchInput && (
                  <li
                    className={
                      this.props.multiSelect || this.props.expandedMultiSelect
                        ? "liSearchWrapper liSearchWrapperMultiSelect"
                        : "liSearchWrapper"
                    }
                    style={{ flex: "1 1 auto" }}
                  >
                    <SearchInput
                      ...
                      multiSelect={
                        ...
                        this.props.expandedMultiSelect === true
                      }
        ...
        >
          {this.state.simpleBarWrapperRef &&
            (propperList.length > 0 || this.state.search.length > 0) &&
            (!this.toggleable || (this.toggleable && this.state.isOpen)) &&
            !this.props.hideList && (
              <MultiDropDownList
                //General props
                list={propperList}
