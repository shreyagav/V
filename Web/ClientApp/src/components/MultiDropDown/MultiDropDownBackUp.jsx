...
class MultiDropDown extends React.Component {
    ...
    if (!this.props.expandBy) {
      if (this.props.multiSelect) {
        ...
        this.props.defaultValue.forEach((value) => {
          headerList.push({
            parentElement: this.props.list.find(
              (el) => value === el[this.props.textProperty],
            ),
          });
        });
      } else {
        // one level 1 value
        let tempElement = this.props.list.find(
          (element) =>
      ...
      if (
        ...
        parentElement = this.props.list.find((listElement) =>
          findChild(listElement),
        );
  ...
  checkIfInnerCheckboxChecked = (index, innerIndex) => {
    ...
    let checked = this.props.defaultValue.indexOf(
      subElement[this.props.expandedKeyProperty],
    );
    if (checked > -1) {
      return true;
    } else return false;
    ...
    if (this.simpleBarWrapperRef !== null) {
      ...
      if (this.dropDownHeaderRef !== null) {
        dropDownHeaderHeight =
          this.dropDownHeaderRef.getBoundingClientRect().height;
      ...
      if (this.toggleable) {
        ...
        if (this.props.substractHeight) {
          if (simpleBarHeight < regularHeight + this.props.substractHeight) {
            simpleBarHeight =
              Math.floor((simpleBarHeight - this.props.substractHeight) / 45) *
                45 -
              1;
          } else {
  ...
  handleWheel = (e) => {
    if (this.simpleBarRef === null || !this.simpleBarRef.contains(e.target)) {
      if (this.toggleable && this.state.isOpen) {
        this.toggle();
      }
      return;
    }
      ...
      case 13: //ENTER
        if (this.props.expandBy) {
          ...
          if (innerIndex > -1) {
            ...
            if (!this.props.expandedMultiSelect) {
              this.props.onDropDownValueChange(
                element[this.props.expandedKeyProperty],
              );
              this.toggle();
            }
          ...
          if (
            ...
            this.setState({
              ...
              setFocusToInnerIndex:
                this.props.list[index - 1].chapters.length - 1,
            });
          } else {
        ...
        >
          {this.props.multiSelect && this.props.expandBy && (
            <label>
              ...
              />
              {checked === true || checked === 1 || checked === -1 ? (
                <CheckBoxSVG />
              ) : (
                <CheckBoxSquareSVG />
              )}
          ...
          <button
            onClick={() => {
              if (this.props.expandBy) {
                ...
                if (!this.props.multiSelect) {
                  this.setState(() => ({
                    value: element[this.props.keyProperty],
                  }));
                  this.props.onDropDownValueChange(
                    element[this.props.keyProperty],
                  );
                  this.toggle();
                } else {
                  this.checkBoxSelected(index, -1);
            ...
            {element[this.props.expandBy].map((el, innerIndex) => (
              <li key={innerIndex} className="openChapter">
                <div
                  ...
                  onKeyDown={(e) => {
                    this.keyDownHandler(e, index, innerIndex, el);
                  }}
                >
                  <button
                    onClick={() => {
                      if (this.props.expandedMultiSelect) {
                        ...
                        this.setState(() => ({
                          value: el[this.props.expandedKeyProperty],
                        }));
                        this.props.onDropDownValueChange(
                          el[this.props.expandedKeyProperty],
                        );
                        this.toggle();
                      }
                    }}
                    ...
                    {this.props.expandedMultiSelect && (
                      <label>
                        <input
                          ...
                          disabled
                          checked={
                            this.checkIfInnerCheckboxChecked(index, innerIndex)
                              ? true
                              : false
                          }
                          /*onChange={() => { 
  ...
  render() {
    const setStyle = () => {
      if (this.toggleable === true) {
        if (this.state.isOpen) {
          return { border: "1px solid #0099cc" };
        }
      } else {
        return {
        ...
        >
          {!this.props.hideList && (
            <div
              ...
              style={
                this.state.isOpen || !this.toggleable
                  ? this.style
                  : { display: "none" }
              }
              ...
              >
                {this.props.expandBy || this.props.list.length < 100 ? (
                  this.props.list.map((element, index) =>
                    this.renderListItem(element, index),
                  )
                ) : (
                  <VirtualList
                    width="100%"
                    height={
                      this.simpleBarHeight.slice(
                        0,
                        this.simpleBarHeight.length - 2,
                      ) * 1
                    }
