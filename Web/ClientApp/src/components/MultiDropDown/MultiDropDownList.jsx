...
class MultiDropDownList extends React.Component {
  ...
  componentDidUpdate() {
    this.setHeight();
    if (
      (this.props.toggleable && this.props.isOpen) ||
      (!this.props.toggleable && this.props.isActive)
    ) {
      this.setFocus();
    }
  ...
  handleClick(e) {
    if (
      this.props.dropDownHeaderRef !== null &&
      this.props.simpleBarWrapperRef !== null &&
      !(
        this.props.dropDownHeaderRef.contains(e.target) ||
        this.props.simpleBarWrapperRef.contains(e.target)
      )
    ) {
      this.props.toggle();
    }
  ...
  setHeight() {
    if (this.props.simpleBarWrapperRef !== null) {
      ...
      if (this.state.style.height !== style.height) {
        /* wrong height, set a right one */
            ...
            {element[this.props.expandBy].map((el, innerIndex) => (
              <li key={innerIndex} className="openChapter">
                <div
                  ref={(el) => {
                    if (
                      index === this.props.setFocusToIndex &&
                      innerIndex === this.props.setFocusToInnerIndex
                    ) {
                      this.props.setFocusToRef(el);
                      this.onFocusRef = el;
                    }
  ...
  render() {
    return (
      <div
        ...
        style={
          this.props.isOpen || !this.props.toggleable
            ? this.state.style
            : { display: "none" }
        }
      >
        ...
        <div style={{ height: "100%" }} data-simplebar>
          <ul
            className={
              "drop-down-list" + (this.props.list.length < 100 ? "" : " pr-0")
            }
