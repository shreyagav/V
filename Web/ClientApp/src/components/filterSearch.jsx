const filterSearch = (list, filter, keys) => {
    ...
    let checkIfElementMeetsCriteria = (el, filterList) => {
        let elementKeys = [];
        if (keys !== undefined) {elementKeys = keys}
        else {
            elementKeys = Object.keys(el);
            let idIndex = elementKeys.indexOf('id');
            if (idIndex > -1) { elementKeys.splice(idIndex, 1) }
        }
        let x = filterList.find(elFilter => {
            return (elementKeys.find(key => { 
    ...
    list.forEach(element => {
        if(checkIfElementMeetsCriteria(element, filterList)){ filteredList.push(element)  }
    })
    return filteredList
} 
