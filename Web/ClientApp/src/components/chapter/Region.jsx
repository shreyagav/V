...
class Region extends Component {
    ...
    componentWillMount() {
        let regionValidators = [
            { 'name': 'regionName', 'typeFunction': 'notEmptyString', 'text': 'Region Name' },
    ...
    componentDidMount() {
        if (this.state.region.regionId != 0) {
            Service.getRegionById(this.state.region.regionId)
    ...
    renderHeader() {
        if (this.props.match.path == '/new-region') {
            return (<h1 className='uppercase-text mb-2'>New<strong> Region</strong></h1>)
        } else {
            return (<h1 className='uppercase-text mb-2'>Edit<strong> Region</strong></h1>)
        }
    }

    performIfValid(callback) {
        if (this.props.store.isFormValid(this.validators, this.state.region)) { callback() }
        else { this.setState({ showError: true }) };
    }

    saveRegion() {
        Service.saveRegion(this.state.region).then(data => { this.setState({ showSuccessfullySavedDialog: true }); }).catch(data => alert(data));
    }
            ...
            <div className='pages-wsm-wrapper ipw-600'>
                ...
                {<div style={{ "width": "100%", "maxWidth": "600px" }}>
                    <ul className='input-fields first-child-text-165 mt-1 pl-1 pr-1'>
                        <li>
                            <p>Region Name:</p>
                            <div className={this.props.store.checkIfShowError('regionName', this.validators) ? 'error-input-wrapper' : ''}>
                                <InputWithClearButton
                                    ...
                                    onChange={e => {
                                        ...
                                        this.props.store.updateValidators("regionName", e.target.value, this.validators);
                                    }}
                                    onClearValue={() => {
                                        this.updateRegiobProperty("regionName", "");
                                        this.props.store.updateValidators("regionName", "", this.validators);
                                    }}
                                />
                                {this.props.store.displayValidationErrors('regionName', this.validators)}
                            </div>
                        </li>
                        <li>
                            <p>Short Name:</p>
                            <div className={this.props.store.checkIfShowError('shortName', this.validators) ? 'error-input-wrapper' : ''}>
                                <InputWithClearButton
                                    type='text'
                                    ...
                                    onChange={e => {
                                        ...
                                        this.props.store.updateValidators("shortName", e.target.value, this.validators);
                                    }}
                                    onClearValue={() => {
                                        this.updateRegionProperty("shortName", "");
                                        this.props.store.updateValidators("shortName", "", this.validators);
                                    }}
                                />
                                {this.props.store.displayValidationErrors('shortName', this.validators)}
                            </div>
                        </li>
                    </ul>
                </div>
                ...
                {this.state.showDeleteDialog &&
                    <Alert
                        ...
                        onOkButtonClick={() => { this.deleteRegion() }}
        ...
        >
                ...
                {this.state.showError &&
                    <Alert
                        ...
                        mode='error'
                    >
                        <h4 className='mb-05'>Error while deleting region {this.state.region.name}</h4>
