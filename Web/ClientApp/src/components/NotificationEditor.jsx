...
export default class EventEditor extends Component {
    ...
    reloadMembers(attendiesOnly) {
        if (attendiesOnly) {
            Service.getEventAttendees(this.props.eventId)
                .then(data => this.setState({ members: data }));
        } else {
            Service.getSiteMembersOnly(this.props.chapterId)
                .then(data => this.setState({ members: data.filter(a=>a.email && a.email!="") }));
        }
    }
    ...
    render() {
        ...
        return (
            <div className='pages-wsm-wrapper ipw-600'>
                <h2 className='uppercase-text mb-2'>New<strong> Email</strong></h2>
                        ...
                        <li>
                            ...
                            <div>
                                <div className='flex-wrap justify-space-between align-center mr-075 ml-05 mb-05'>
                                    <div className='flex-wrap justify-left'>
                                        <CheckBox
                                            ...
                                            onClick={() => {
                                                this.reloadMembers(!this.state.attendiesOnly)
                                                this.setState({ attendiesOnly: !this.state.attendiesOnly });
                                            }}
                                ...
                                {members.length > 0 &&
                                    ...
                                    onDropDownValueChange={value => this.props.updateNotification("recepients", [...value])}
                                        hideHeader={true}
                                        substractHeight={400}
                        ...
                        <li>
                            ...
                            <div className='input-button-wrapper'>
                                <textarea
                                    ...
                                    onChange={(evt) => this.props.updateNotification("body", evt.target.value)}
                                />
                            </div>
                        </li>
                    </ul>
