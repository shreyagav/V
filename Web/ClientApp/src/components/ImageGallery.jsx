...
class ImageGallery extends Component {
        ...
        for (let j = 0; j < newArray.length; j++) {
          newArray[j].flexBasis =
            ((newArray[j].width * rowHeight) /
              ...
              totalWidth) *
            100 *
            (1 - (this.state.margin * (counter - 1)) / maxWidth);
        }
  ...
  render() {
    ...
    const amountOfPages = Math.ceil(
      this.props.pictures.length / this.state.amountPerPage,
    );
    return (
      <div className="flex-wrap align-center justify-center">
        <ul
          ref={(el) => (this.imageGalleryRef = el)}
          style={{ width: "100%" }}
          className="image-gallery mt-2 mb-2"
        >
          {formattedList.map((element, index) => (
            <li
              key={index}
              style={
                index < formattedList.length - 1
                  ? { marginBottom: this.state.margin + "px" }
                  : { marginBottom: "0px" }
              }
            >
                  ...
                  style={
                    index < element.length - 1
                      ? {
                          ...
                          flexBasis: innerElement.flexBasis + "%",
                        }
                      : { flexBasis: innerElement.flexBasis + "%" }
                  }
                  tabIndex="0"
        ...
        <Pagination
          ...
          setPageNumber={(number) => {
            ...
            this.formatImageList(this.props.pictures, number);
          }}
        />
      </div>
    );
