...
class NavMenu extends Component {
  ...
  signOut() {
    ...
    fetch("/api/Account/SignOut")
      ...
      .then(function (jjson) {
        if (jjson.result == "ok") me.props.store.set("userInfo", null);
    ...
    return (
      <header className="main-nav-wrapper">
        ...
        <ul className="flex-nowrap main-nav">
          ...
          {this.props.match.path != "/chapters" &&
            user &&
            user.authType == "Admin" && (
              <li hint="Chapters">
                <Link to="/chapters">
          ...
          {this.props.match.path != "/events" &&
            user &&
            (user.authType == "Admin" || user.authType == "Secretary") && (
              <li hint="Events">
                <Link to="/events">
                  <EventSVG />
                  <span>Events</span>
        ...
        <ul className="flex-nowrap main-nav">
          {this.props.match.path != "/profile" && user && (
            <li hint="Profile">
              <Link to="/profile">
                <UserSVG />
                <span
                  className="sign-in ml-05"
          ...
          {user == null && (
            <li hint="Sign In">
              <Link to="/SignIn">
                <SignInSVG />
                <span className="sign-in" style={{ textTransform: "none" }}>
                  Sign In
                </span>
              </Link>
            </li>
