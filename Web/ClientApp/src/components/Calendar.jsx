...
class Calendar extends Component {
  ...
  getEventsForMonth(year, month, sites) {
    ...
    Service.getCalendarEvents(month + 1, year, sites)
      .then((data) => component.setState({ events: data, loading: false }))
      .catch((err) => component.setState({ loading: false }));
  }
  ...
  calendarUpdate = () => {
    if (this.props.store.tableStileView) {
      ...
      let newCalendar = this.state.calendar.filter((element) => {
        return element.className === "current-month";
      });
      return newCalendar;
    }
  };
        ...
        <div>
          <div className="flex-flow-column mr-1 ml-1">
            <div className="flex-wrap justify-space-between align-center">
              <div className="flex-wrap justify-left align-center">
                ...
                {this.props.store.narrowScreen && (
                  <TabComponent
                    ...
                    tabList={["table", "list"]}
                    wasSelected={(index) =>
                      this.props.store.set("tableStileView", index === 0)
                    }
              ...
              <span>
                {!(
                  this.state.currentMonth === this.todayMonth &&
                  this.state.currentYear === this.todayYear &&
              ...
              <div style={{ marginLeft: "-0.5em", marginRight: "-0.5em" }}>
                <MultiDropDown
                  ...
                  onDropDownValueChange={(value) =>
                    this.props.onBodyDropDownValueChange(value)
                  }
                  hideHeader={false}
                  hideList={true}
                />
          ...
          <div className="flex-nowrap justify-stretch m-1 align-center">
            <button
              className="h1 square-button-height"
            ...
            <button
              ...
              disabled={this.state.regularCalendar ? false : true}
            >
              <h1 className="uppercase-text" style={{ paddingBottom: "0.2em" }}>
                {this.state.regularCalendar &&
                  monthNames[this.state.currentMonth] + " "}
                <strong>
                  <b>{this.state.currentYear}</b>
                </strong>
              </h1>
            </button>
            <button
          ...
          {this.state.regularCalendar && (
            <ul className="calendar-grid calendar-content dark-grey-text">
              {calendar.map((element, index) => {
                let eventKey =
                  (element.date.getMonth() + 1).toString() +
                  "-" +
                  element.date.getDate().toString();
                let dayOfEvents = this.state.events.find(
                  (a) => a.day == eventKey,
                );
                return (
                  <li
                    ...
                    className={
                      this.props.store.tableStileView
                        ? element.className
                        : element.className + " listStyleView"
                    }
                    tabIndex="0"
                      ...
                      <div className={element.className}>
                        ...
                        {dayOfEvents && (
                          <ul className="calendar-events-list">
                            {dayOfEvents.events.map((event, index) => (
                              <li
                                className="blue-link"
                                key={index}
                                onClick={(e) =>
                                  this.navigateToEventView(event.id)
                                }
                              >
                                <span
                                  className="blue-link-invert"
                                  style={{ backgroundColor: event.color }}
                                >
                                  {event.hours.toString() +
                                    ":" +
                                    ("0" + event.minutes.toString()).slice(-2) +
                                    " " +
                                    (event.am ? "AM" : "PM")}
                                </span>
                                <span
                              ...
                              {dayOfEvents.events.map((event, index) => (
                                ...
                                >
                                  ...
                                  >
                                    {event.hours.toString() +
                                      ":" +
                                      ("0" + event.minutes.toString()).slice(
                                        -2,
                                      ) +
                                      " " +
                                      (event.am ? "AM" : "PM")}
