const storeValidatorsRules = {
  ...
  nameOTG: (text) => {
    let message1 = text + " includes invalid characters";
    return [{ test: /^$|^[^#&<>\"~;$^%{}?]{1,}$/, message: message1 }];
  },
  phone: (text) => {
  ...
  phoneOTG: (text) => {
    ...
    return [{ test: /^(\(?\+?[0-9]*\)?)[0-9\.\- \(\)]*$/, message: message1 }];
  },
  email: (text) => {
    let message1 = text + " is not valid";
    return [{ test: /^.+@[^\.].*\.[a-z]{2,}$/, message: message1 }];
  },
  emailOTG: (text) => {
    let message1 = text + " is not valid";
    return [
  ...
  password: (text) => {
    ...
    let message2 =
      text + " must contain at least one non alphanumeric character";
    let message3 =
      text + " must contain at least one lowercase ('a'-'z') character";
    let message4 =
      text + " must contain at least one uppercase ('A'-'Z') character";
  ...
  zip: (text) => {
    let message1 = text + " is not valid";
    return [{ test: /^\d{1,5}(?:[-\s]\d{0,4})?$/, message: message1 }];
  },
  zipOTG: (text) => {
    let message1 = text + " is not valid";
    return [{ test: /^\d{1,5}(?:[-\s]\d{0,4})?$/, message: message1 }];
