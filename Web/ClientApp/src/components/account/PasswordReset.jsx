...
class PasswordReset extends Component {
    ...
    checkIfPasswordRepeatValid = (field) => {
        if(this.validators[field].activated === false && this.state.passwordReset[field] !== ''){
            this.props.store.updateValidators(field, this.checkIfMatches(this.state.passwordReset[field], this.state.passwordReset.NewPassword), this.validators);
    ...
    checkIfFieldValid = (field) => {
        if(this.validators[field].activated === false && this.state.passwordReset[field] !== ''){
            this.props.store.updateValidators(field, this.state.passwordReset[field], this.validators);
            let tmp = this.state.passwordReset;
            this.setState({ passwordReset: tmp });
        }
    }
    ...
    changeNewPassword(e, field){
        ...
        if(this.validators["NewPasswordRepeat"].activated){
            ...
            this.props.store.updateValidators("NewPasswordRepeat", match, this.validators);
        }
    ...
    changeNewPasswordRepeat = (e) => {
        /* check validation only if field was previously activated! */
        if(this.validators["NewPasswordRepeat"].activated){
            this.props.store.updateValidators("NewPasswordRepeat", this.checkIfMatches(e.target.value, this.state.passwordReset.NewPassword), this.validators);
        }
        this.handleChange(e);
    }
    ...
    submitChangePasswordInfo() {
            ...
            fetch('/api/account/SendResetPasswordToken', {
                ...
                .then(json => {
                    if (json.error != null) {
                        ...
                        this.setState({ loading: false, message: "Email with further instructions was sent." });
                    }
                })
        ...
        if (this.state.passwordReset.Token) {
            ...
            fetch('/api/account/ResetPassword', {
                ...
                .catch(err => this.setState({ loading: false, error: err }));
        } else this.props.store.performIfValid(this.state.passwordReset, this.validators, sendToken, showError);
    }
        ...
        return (
            ...
            <div>
                <div className = 'flex-nowrap justify-space-between mb-2 w-100'>
                    <h3 className="uppercase-text mb-05">Reset<strong> Password</strong></h3>
                    <a href="http://www.teamriverrunner.org" target="_self" style={{ "height": "1.5em" }}><LogoSVG /></a>
                </div>
                        ...
                        {this.state.passwordReset.Token && <section>
                            ...
                            <li>
                                ...
                                <PasswordComponent
                                    ...
                                    onChange = {(e, name) => this.changeNewPasswordRepeat(e, name)}
                ...
                <div className = 'flex-wrap justify-center mb-2 w-100'>
                    {!this.state.message && 
                        <button 
                            type="button" className='medium-static-button static-button' 
                            style={{ width: "auto", paddingRight: "1rem", paddingLeft: "1rem" }} 
                            onClick={this.submitChangePasswordInfo}>{this.state.passwordReset.Token ? "Reset Password" : "Send reset password link"}
                        </button>
                    }
