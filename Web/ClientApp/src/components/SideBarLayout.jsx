...
class SideBarLayout extends Component {
  ...
  moveTouch(e) {
    ...
    if (Math.abs(diffX) > Math.abs(diffY)) {
      ...
      if (diffX > 0) {
        ...
        // swiped right
        if (
          this.bodyRef.contains(e.target) &&
          this.initialX < 100 &&
          this.props.store.sideBarIsHidden &&
          !this.longTouch
        ) {
  ...
  mainLevelStyleUpdate = () => {
    if (this.props.store.narrowScreen) {
      if (this.props.store.sideBarIsHidden) {
        return { paddingLeft: "0rem", paddingRight: "0rem" };
      } else
        return { paddingLeft: "0rem", paddingRight: "0rem", position: "fixed" };
    } else {
      return { paddingLeft: "325px", paddingRight: "0rem" };
    }
  };
  ...
  render() {
    ...
    return (
      <div className={"lm-wrapper overflow-hidden"}>
        <div
          ...
          style={
            this.props.store.narrowScreen
              ? this.props.store.sideBarIsHidden
                ? { left: "-325px", backgroundColor: "#ffffff" }
                : { left: "0px", backgroundColor: "#ffffff" }
              : {
                  left: "0px",
        ...
        <div
          className={
            this.props.store.narrowScreen && !this.props.store.sideBarIsHidden
              ? "black-layer-visible"
