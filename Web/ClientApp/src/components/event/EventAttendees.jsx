...
class EventAttendees extends Component {
  ...
  submitMembersToEvent() {
    ...
    Service.addEventAttendees(this.state.eventId, this.state.tempMembers).then(
      (data) =>
        this.setState({
          members: data,
          tempMembers: [],
  ...
  addExistingMember() {
    this.setState({ loading: true });
    Service.getSiteMembers(this.state.eventId).then((data) => {
      this.setState(
        {
          siteMembers: data,
          loading: false,
  ...
  removeMember(member) {
    var me = this;
    this.setState({ loading: true });
    Service.removeEventAttendee(this.state.eventId, member).then((data) => {
      setTimeout(() => {
  ...
  filterMemberList = () => {
    ...
    let checkIfElementMeetsCriteria = (el, filterList) => {
      ...
      filterList.forEach((elFilter) => {
        if (
          !(
            el.firstName.toLowerCase().includes(elFilter) ||
            el.lastName.toLowerCase().includes(elFilter) ||
            el.email.toLowerCase().includes(elFilter) ||
            el.phone.toLowerCase().includes(elFilter)
          )
        ) {
          meetsCriteria = false;
        }
    ...
    list.forEach((element) => {
      if (
        ((filterList.length > 0 &&
          checkIfElementMeetsCriteria(element, filterList)) ||
          filterList === "") &&
        (this.state.activeMembersOnlyChecked === false ||
          (this.state.activeMembersOnlyChecked === true &&
            element.active === true)) &&
        ((this.state.chapterOnlyMembers &&
          element.siteId == this.props.chapterId) ||
          !this.state.chapterOnlyMembers) &&
        (this.state.selectedMembersOnlyChecked === false ||
          (this.state.selectedMembersOnlyChecked === true &&
            this.state.tempMembers.indexOf(element.id) > -1))
      ) {
  ...
  renderFullNameColumn(value, row, index, col) {
    return (
      ...
      >
        ...
        <span style={{ flex: "1 1 auto" }} className="big-bold">
  ...
  render() {
    ...
    const columns = [
      {
        ...
        render: this.renderFullNameColumn,
      },
    ...
    return (
      <div className="w-100 prpl-0">
          ...
          <FixedWrapper maxWidth={"600px"} noPadding={true}>
            ...
            <div className="flex-wrap justify-space-between align-center mr-075 ml-05 mb-05">
              ...
              <CheckBox
                ...
                onClick={() => {
                  this.setState(
                    { chapterOnlyMembers: !this.state.chapterOnlyMembers },
                    this.filterMemberList,
                  );
                }}
              ...
              <div className="flex-wrap justify-left">
                <CheckBox
                  ...
                  onClick={() => {
                    ...
                    this.setState(
                      {
                        activeMembersOnlyChecked:
                          !this.state.activeMembersOnlyChecked,
                        selectAllCheckboxChecked: false,
                        //tempMembers: [],
                      },
                      this.createFilteredListWithTimeOut(),
                ...
                <CheckBox
                  ...
                  onClick={() => {
                    ...
                    this.setState(
                      {
                        selectedMembersOnlyChecked:
                          !this.state.selectedMembersOnlyChecked,
                      },
                      this.createFilteredListWithTimeOut(),
                    );
                  }}
                  ...
                  labelText={
                    <span>
                      Selected Only
                      <strong>
                        {" " + this.state.tempMembers.length.toString()}
                      </strong>
                    </span>
                  }
                />
            ...
            {this.state.filteredList.length === 0 && (
              <p className="message-block mb-2 mt-2">
                There are no members that meet this criteria.
              </p>
            )}
            {this.state.filteredList.length > 0 && (
              <MultiDropDown
                ...
                onDropDownValueChange={(value) =>
                  this.setState({ tempMembers: value })
                }
                hideHeader={true}
                substractHeight={65}
                textPropertyRender={(element, textProperty) =>
                  this.textPropertyRender(element, textProperty)
                }
              />
            )}
        ...
        {members.length === 0 && (
          <p className="message-block mb-2 pr-1 pl-1">
          ...
          <div className="flex-wrap align-center justify-center mb-2 pr-1 pl-1">
            ...
            <span>
              ...
              <button
                ...
                onClick={this.addExistingMember}
              >
                Add Members
              </button>
            </span>
