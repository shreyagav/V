...
class EventNotifications extends Component {
  ...
  componentDidMount() {
    ...
    Service.getEventNotification(this.props.eventId).then((data) =>
  ...
  submitNotification() {
    ...
    not.recepients = not.recepients.map((a) => {
      return { userId: a };
    });
    Service.addEventNotification(not).then((data) =>
      this.setState({
        notifications: data,
        newNotification: {
          subject: "",
      ...
      },
      {
        ...
        render: (e, a) => (
          <li className="table-content">
            <span className="table-mini-header">Recepients:</span>
            <span>{e ? e.length : 0}</span>
          </li>
        ),
      },
      {
        ...
        render: (a) => (
          <li className="table-content">
            <span className="table-mini-header">Created:</span>
            <span>{this.getDateTime(a)}</span>
          </li>
        ),
      },
      {
        ...
        render: (colVal) => (
          <li className="table-content">
            <span className="table-mini-header">Created By:</span>
            <span>
              {colVal.firstName} {colVal.lastName}
            </span>
          </li>
        ),
      },
      ...
      <div className="w-100 prpl-0">
        ...
        {notifications.length === 0 && (
          <p className="message-block mb-2 pr-1 pl-1">
            There are no messages registered for the event.
          </p>
        )}
