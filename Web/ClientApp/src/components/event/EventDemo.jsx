...
class EventDemo extends Component {
  ...
  componentDidMount() {
    ...
    if (this.state.eventId != 0) {
      Service.getEvent(this.state.eventId)
        ...
        .catch((exception) =>
          component.setState({ error: exception, loading: false }),
        );
    }
    fetch("/Pictures.json")
      .then(function (data) {
  ...
  formatImageList(pageNumber) {
    ...
    for (
      let i = (pageNumber - 1) * this.state.amountPerPage;
      i < tryUntill;
      i++
    ) {
      totalWidth =
        totalWidth + (imageList[i].width * rowHeight) / imageList[i].height;
      counter = counter + 1;
      newArray.push(imageList[i]);
      if (totalWidth > maxWidth) {
        for (let j = 0; j < newArray.length; j++) {
          newArray[j].flexBasis =
            ((newArray[j].width * rowHeight) /
              newArray[j].height /
              totalWidth) *
            100 *
            (1 - (10 * (counter - 1)) / maxWidth);
        }
    ...
    if (newArray.length > 0) {
      for (let j = 0; j < newArray.length; j++) {
        newArray[j].flexBasis =
          ((newArray[j].width * rowHeight) / newArray[j].height / maxWidth) *
          100 *
          (1 - (10 * (counter - 1)) / maxWidth);
      }
      newImageList.push(newArray);
  ...
  onAttendanceChanged(index) {
    ...
    func(this.state.eventMain.id)
      .then((data) => this.setEventMain(data, true))
      .catch((err) => alert(err));
  }
  ...
  getStartAndEnd(date, from, to) {
    var start = new Date(
      date.getTime() +
        ((from.am ? 1 : 12) + from.hours) * 60 * 60 * 1000 +
        from.minutes * 60 * 1000,
    )
      .toISOString()
      .replace(/[\:\-\.]/gi, "");
    var end = new Date(
      date.getTime() +
        ((to.am ? 1 : 12) + to.hours) * 60 * 60 * 1000 +
        to.minutes * 60 * 1000,
    )
      .toISOString()
      .replace(/[\:\-\.]/gi, "");
  ...
  render() {
    ...
    if (this.props.store.chapterList.length > 0 && !this.state.loading) {
      let state = this.props.store.chapterList.find((element) => {
        return element.chapters.find((innerElement) => {
          return innerElement.id === this.state.eventMain.site;
        });
      });
      chapter = state.chapters.find((element) => {
        return element.id === this.state.eventMain.site;
      });
    }
    if (this.state.eventMain.date !== undefined) {
      var options = { day: "2-digit", month: "2-digit", year: "numeric" };
      var { start, end } = this.getStartAndEnd(
        this.state.eventMain.date,
        this.state.eventMain.timeFrom,
        this.state.eventMain.timeTo,
      );
      eventDate = this.state.eventMain.date.toLocaleDateString(
        "en-US",
        options,
      );
      eventTime =
      ...
      if (this.props.store.eventTypes.length > 0) {
        eventType = this.props.store.eventTypes.find(
          (element) => element.id === this.state.eventMain.eventType,
        ).title;
      }
    ...
    return (
      <div className="pages-wsm-wrapper ipw-1000">
        ...
        <div className="second-nav-wrapper">
          <div className="ipw-1000">
            <div className="flex-nowrap justify-center align-center">
              {this.state.eventMain.date > new Date() && (
                <div className="flex-nowrap justify-center align-center">
                  <span
                    style={{
                  ...
                  <TabComponent
                    ...
                    activeTabIndex={
                      this.state.eventMain.curentUserAttends ? 0 : 1
                    }
                  />
                </div>
              )}
            ...
            <div>
              {user &&
                (user.authType == "Admin" ||
                  (user.authType == "Secretary" &&
                    user.chapterId == this.state.eventMain.site)) && (
                  <button
        ...
        <h2 className="flex-nowrap align-center mb-2">
              ...
              <ul className="icon-text-set mb-1">
                ...
                <li>
                  ...
                  <span className="chapter">
                    {chapter.name !== undefined && chapter.name}
                  </span>
                </li>
                <li>
                  <EventUpSVG />
        ...
        {this.state.activeTabIndex === 1 && (
          <EventAttendees
            ...
            attendees={this.state.members}
            chapterId={this.state.eventMain.site}
            editsPermitted={false}
            showAttended={
              user &&
              (user.authType == "Secretary" || user.authType == "Admin") &&
              this.state.eventMain.date < new Date()
            }
          />
        )}
        ...
        {this.state.activeTabIndex === 4 && (
          <EventPictures eventId={this.state.eventId} editsPermitted={false} />
        )}
        {!(
          user &&
          (user.authType == "Admin" ||
