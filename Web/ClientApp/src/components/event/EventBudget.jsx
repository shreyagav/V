...
class EventBudget extends Component {
  ...
  componentDidMount() {
    ...
    Service.getBudget(this.state.eventId).then((data) =>
      this.setState({
        budget: data,
        loading: false,
        calculatedCost: this.getCalculatedCost(data),
      }),
    );
  }
  ...
  addLine() {
    ...
    if (line.id === undefined) {
      Service.addBudgetLine(this.state.eventId, this.state.line).then(thenFunc);
    } else {
      Service.updateBudgetLine(this.state.eventId, this.state.line).then(
        thenFunc,
      );
    }
  }
  ...
  removeItem() {
    //removed parameter b/c now line to delete is state.line
    this.setState({ loading: true });
    Service.deleteBudgetLine(this.state.eventId, this.state.line).then((data) =>
      this.setState({
        loading: false,
        line: {},
        removingLine: false,
    ...
    return (
      ...
      >
        <div style={{ flex: "1 1 auto" }}>
          ...
          <span style={{ color: "#0099cc" }} className="big-bold">
            {row["quantity"]}
          </span>
        </div>
        {this.props.editsPermitted !== false && (
          <button
            ...
            onClick={() => {
              this.setState({ removingLine: true, line: row });
            }} //added line:row to define which line is going to be removed
          >
            <CloseUpSVG />
          </button>
        )}
  ...
  addLineValidationPassed() {
    ...
    if (
      ...
      this.state.line.name === null ||
      this.state.line.name === undefined
    ) {
      this.emptyName = true;
      ...
      <div style={{ width: "100%", maxWidth: "600px" }}>
        ...
        {this.state.addingLine && (
          <FixedWrapper maxWidth={"600px"}>
            ...
            <ul className="input-fields first-child-text-110 mt-1">
              <li
                className={this.emptyName === true ? "mark-invalid" : ""}
                error-text="Please enter Item Name"
              >
                <p>Name:</p>
              ...
              <li>
                ...
                <div className="input-button-wrapper">
                  <textarea
                    ...
                    onChange={(e) =>
                      this.changeLineProperty("description", e.target.value)
                    }
                  />
                </div>
              </li>
                ...
                <ul className="input-fields-child-ul justify-space-between flex-wrap">
                  ...
                  >
                    <div className="input-button-wrapper">
                      ...
                      {this.state.line.quantity > 1 && (
                        <button
                          onClick={() => this.changeLineProperty("quantity", 1)}
                        >
                          <CloseUpSVG />
                        </button>
                      )}
                  ...
                  {this.state.line.cost > 0 && this.state.line.quantity > 0 && (
                    <li style={{ flex: "0 0 auto" }}>
                      <p style={{ marginRight: "0em" }}>
                        Total: $
                        {(
                          this.state.line.cost * this.state.line.quantity
                        ).toFixed(2)}
                      </p>
                    </li>
                  )}
        ...
        {this.state.removingLine && (
          <Alert
            ...
            onCancelButtonClick={() =>
              this.setState({ removingLine: false, line: {} })
            } //added line:null
            ...
            <ul className="flex-wrap justify-center">
              {this.state.line.cost > 0 && (
                <li className="flex-nowrap mr-05 ml-05 mb-05">
                  ...
                  <span className="bold-text">
                    <strong>{"$" + this.state.line.cost}</strong>
                  </span>
                </li>
              )}
              <li className="flex-nowrap mr-05 ml-05 mb-05">
                ...
                <span className="bold-text">
                  <strong>{this.state.line.quantity}</strong>
                </span>
              </li>
              {this.state.line.quantity > 1 && (
                <li className="flex-nowrap mr-05 ml-05 mb-05">
        ...
        <ul className="input-fields first-child-text-240 mb-1 pl-1 pr-1">
          {(!(
            this.props.editsPermitted === false &&
            parseFloat(this.props.projectedCost) === 0
          ) ||
            parseFloat(this.props.projectedCost) > 0) && (
            <li>
          ...
          {budget.length > 0 && (
            <li>
              ...
              <input
                ...
                style={{
                  color: "rgb(0, 153, 204)",
                  /*"borderColor":"#fcfcfc",*/ backgroundColor: "#f4f3f2",
                }}
              />
            </li>
          )}
        ...
        {(budget.length === 0 || budget.length === undefined) && (
          <p className="message-block mb-2">
            There are no items yet. Add Items to the list to calculate event's
            budget.
          </p>
        )}
        {this.props.editsPermitted !== false && (
          <div className="flex-wrap align-center justify-center mb-2">
            ...
            <button
              ...
              onClick={() =>
                this.setState({ addingLine: true, headerText: "Add Item" })
              }
            >
              Add Item
            </button>
