...
class VeteransBySite extends Component {
  ...
  updateData() {
    ...
    Service.getVeteransBySiteReport(this.state.range).then((data) =>
  ...
  donwloadExcel() {
    this.setState({ loading: true });
    Service.downloadWithPost(
      "/api/Reports/VeteransBySiteToExcel",
      this.state.range,
    ).then((blob) => {
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        Header: (
          <span>
            Attendance, Total:<b>{totalAttandance}</b>
          </span>
        ),
        accessor: "attendance",
        filterable: false,
    ...
    return (
      <div>
        ...
        <h3 className="mr-05 ml-05 mt-2 mb-2 uppercase-text">
          <strong>Veterans</strong> by Chapter
        </h3>
        {/*<div>    
                {this.state.loading && <Loader/>}
