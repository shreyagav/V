...
class VeteransAttendence extends Component {
  ...
  updateData() {
    ...
    Service.getVeteransAttendence(this.state.range).then((data) =>
  ...
  donwloadExcel() {
    ...
    Service.downloadWithPost(
      "/api/Reports/VeteransAttandanceToExcel",
      this.state.range,
    ).then((blob) => {
    ...
    return (
      <div>
        ...
        <h3 className="mr-05 ml-05 mt-2 mb-2 uppercase-text">
          <strong>Veteran's Attendance</strong> by Date Range
        </h3>
        {/*    {this.state.loading && <Loader />}
