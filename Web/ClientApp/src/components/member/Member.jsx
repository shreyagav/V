...
class Member extends Component {
  ...
  componentWillMount() {
    ...
    this.validators = memberValidators(
      this.state.userId === "" && this.props.match.path !== "/profile",
    );
  ...
  saveMemberInfo() {
    ...
    Service.setProfile(this.state.member)
      .then((data) => {
        if (data === undefined) {
          this.setState({ loading: false, showErrorSaveDialog: true });
        } else {
          this.setState({
            member: this.fixData(data),
            loading: false,
            showSuccessfullySavedDialog: true,
          });
        }
      })
      .catch((er) => {
        ...
        this.setState({
          loading: false,
          showErrorSaveDialog: true,
          messageErrorSaveDialog: mess,
        });
      });
  }
  ...
  renderHeader() {
    if (this.props.match.path == "/profile") {
      ...
      return (
        <h1 className="uppercase-text mb-2">
          New<strong> Member</strong>
        </h1>
      );
    } else {
      return (
        <h1 className="uppercase-text mb-2">
          Edit<strong> Member</strong>
        </h1>
      );
    }
  }
      ...
      <div className="pages-wsm-wrapper ipw-800">
          ...
          <div>
            <ul className="input-fields first-child-text-125 pl-1 pr-1">
              ...
              <li>
                ...
                <div
                  className={
                    this.props.store.checkIfShowError("siteId", this.validators)
                      ? "error-input-wrapper"
                      : ""
                  }
                >
              ...
              <li className="input-wrapper">
                <p
                  className={this.state.member.id == "" ? "mark-optional" : ""}
                >
                  Email:
                </p>
                <div
                  className={
                    ...
                    this.props.store.checkIfShowError("email", this.validators)
                      ? "error-input-wrapper"
                      : ""
                  }
                >
              ...
              <li>
                ...
                <div>
                  <div
                    className={
                      "flex-wrap justify-left radio-inline-wrapper" +
                      (this.props.store.checkIfShowError(
                        "gender",
                        this.validators,
                      )
              ...
              <li>
                ...
                <ul className="input-fields flex-nowrap break-at-560 line-of-inputs-wrapper">
                  <li className="input-wrapper" style={{ flex: "1 1 auto" }}>
                    <input
                      type="text"
                      placeholder="City"
                  ...
                  <li className="input-wrapper" style={{ flex: "1 0 150px" }}>
                    <MultiDropDown
                      ...
                      textPropertyRender={(element, textProperty) =>
                        this.stateNameAndAbbrRender(element, textProperty)
                      }
                      onDropDownValueChange={(value) =>
        ...
        {this.state.activeTabIndex === 2 && (
          <MemberTRRInfo
            ...
            updateMemberProperty={(property, value) =>
              this.updateMemberProperty(property, value)
            }
        ...
        {this.state.showErrorSaveDialog && (
          <Alert
            ...
            showOkButton={true}
            onOkButtonClick={() =>
              this.setState({ showErrorSaveDialog: false })
            }
            okButtonText="Ok"
            mode="error"
          >
            <p className="mb-05" style={{ textAlign: "center" }}>
              {" "}
              {this.state.messageErrorSaveDialog}{" "}
            </p>
            <h4 className="mb-05">
              {this.state.member.firstName + " " + this.state.member.lastName}
            </h4>
