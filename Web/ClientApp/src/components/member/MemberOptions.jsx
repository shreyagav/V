...
class MemberOptions extends Component {
  ...
  onEditOption() {
    ...
    Service.editUserOption(this.props.member.id, this.state.alertContent).then(
      (userOptions) =>
        this.setState({ options: userOptions, showAddEditOption: false }),
    );
    this.elementToEdit = null;
  }
  ...
  onDeleteOption(row) {
    ...
    let el = this.state.optionsList.find(
      (element) => element.categoryId === row.categoryId,
    );
  ...
  removeOption() {
    //TODO: handle loading
    Service.deleteUserOption(this.props.member.id, this.elementToEdit).then(
      (userOptions) =>
        this.setState({ options: userOptions, removeOptionShowAlert: false }),
    );
  ...
  renderOption(value, row, index, col) {
    let category = this.state.optionsList.find(
      (element) => element.categoryId === row.categoryId,
    );
    let option = category.options.find(
      (element) => element.id === row.optionId,
    );
    return (
      <li
        className={
      ...
      >
        <div className="flex-nowrap justify-space-between align-center w-100">
          <div className="flex-nowrap flex-flow-column  justify-left align-self-center">
            <span style={{ fontSize: "1.1em", marginRight: "auto" }}>
  ...
  renderComments(value, row, index, col) {
    return (
      ...
      >
        ...
        <div
          className="flex-nowrap flex-flow-column justify-left align-self-center"
          style={{ width: "100%" }}
        >
  ...
  onAddOption() {
    ...
    //TODO: handle loading
    Service.addUserOption(this.props.member.id, this.state.alertContent).then(
      (userOptions) =>
        this.setState({
          showAddEditOption: false,
          alertContent: this.alertDafaultContent,
          options: userOptions,
        }),
    );
  }
  ...
  render() {
    ...
    if (this.state.alertContent.categoryId !== null) {
      options = optionsList.find(
        (element) => element.categoryId === this.state.alertContent.categoryId,
      ).options;
    }
    let optionsValue = null;
    if (options.length === 1) {
      optionsValue = options[0].id;
      if (this.state.alertContent.optionId !== optionsValue) {
        this.onAlertOptionsChange(optionsValue);
      }
    }
    const columns = [
      {
      ...
      <div className="flex-nowrap flex-flow-column justify-center w-100 prpl-0">
        {this.state.showAddEditOption && (
          <FixedWrapper maxWidth={"600px"}>
              ...
              <li>
                ...
                <MultiDropDown
                  ...
                  onDropDownValueChange={(value) =>
                    this.onAlertCategoryChange(value)
                  }
                />
              </li>
              {this.state.alertContent.categoryId !== null && (
                <li>
                  ...
                  <MultiDropDown
                    ...
                    onDropDownValueChange={(value) => {
                      this.onAlertOptionsChange(value);
                    }}
                  />
                </li>
              )}
              {this.state.alertContent.categoryId !== null && (
                <li>
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      this.onAlertDescriptionChange(e.target.value)
                    }
                  />
                </li>
              )}
            ...
            <div className="flex-nowrap justify-center mt-2 mb-2">
              {this.state.alertContent.optionId !== null &&
                this.state.alertContent.categoryId !== null && (
                  <button
        ...
        {this.state.removeOptionShowAlert && (
          <Alert
            ...
            okButtonText="Delete"
            onCancelButtonClick={() =>
              this.setState({ removeOptionShowAlert: false })
            }
        ...
        {!this.state.showAddEditOption && this.state.options.length === 0 && (
          <p className="message-block mb-2">
            There are no options assigned to this member yet.
          </p>
        )}
        {!this.state
          .showAddEditOption /*this.props.editsPermitted !== false && */ && (
