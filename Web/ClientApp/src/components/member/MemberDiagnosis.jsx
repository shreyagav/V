...
class MemberDiagnosis extends Component {
  ...
  onEditDiagnosis() {
    Service.editUserDiagnosis(
      this.props.member.id,
      this.state.alertContent,
    ).then((userDiagnosis) =>
      this.setState({ diagnosis: userDiagnosis, showAddEditDiagnosis: false }),
    );
    this.elementToEdit = null;
  }
  ...
  removeDiagnose() {
    //TODO: handle loading
    Service.deleteUserDiagnosis(this.props.member.id, this.elementToEdit).then(
      (userDiagnosis) =>
        this.setState({
          diagnosis: userDiagnosis,
          removeDiagnoseShowAlert: false,
        }),
    );
  }
  ...
  renderOption(value, row, index, col) {
    ...
    return (
      ...
      >
        <div
          className="flex-nowrap justify-space-between align-center "
          style={{ width: "100%" }}
        >
  ...
  onAddDiagnose() {
    //TODO: handle loading
    Service.addUserDiagnose(this.props.member.id, this.state.alertContent).then(
      (userDiagnosis) =>
        this.setState({
          showAddEditDiagnosis: false,
      ...
      <div className="flex-nowrap flex-flow-column justify-center w-100 prpl-0">
          ...
          <FixedWrapper maxWidth={"600px"}>
            ...
            <ul className="input-fields first-child-text-95">
              <li>
                <p>Diagnose:</p>
                <MultiDropDown
                  list={this.state.diagnosisList.filter(
                    (a) =>
                      this.state.diagnosis.find((b) => b.id == a.id) == null ||
                      a.id === this.state.alertContent.id,
                  )}
                  multiSelect={false}
                  ...
                  onDropDownValueChange={(value) =>
                    this.onAlertDiagnoseChange(value)
                  }
                />
              </li>
              <li>
        ...
        {this.state.removeDiagnoseShowAlert && (
          <Alert
            ...
            okButtonText="Remove"
            onCancelButtonClick={() =>
              this.setState({ removeDiagnoseShowAlert: false })
            }
        ...
        {!this.state.showAddEditDiagnosis &&
          this.state.diagnosis.length === 0 && (
            <p className="message-block mb-2">
              There are no diagnosis assigned to this member yet.
            </p>
          )}
        {!this.state
          .showAddEditDiagnosis /*this.props.editsPermitted !== false && */ && (
