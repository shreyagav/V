...
class DatePicker extends Component {
  ...
  toggle() {
    ...
    if (this.state.regularCalendar) {
      incButtonDisabled = !this.checkIfMonthIsWalkable(
        currentYear,
        currentMonth,
        true,
      );
      decButtonDisabled = !this.checkIfMonthIsWalkable(
        currentYear,
        currentMonth,
        false,
      );
  ...
  createCalendar(year, month, callback) {
    ...
    //PREVIOUS MONTH
    for (
      let i = prevMonthNumberOfDays - firstDayOfMonth + 1;
      i < prevMonthNumberOfDays + 1;
      i++
    ) {
      calendar.push({
        label: i,
    ...
    for (let i = 0; i < thisMonthNumberOfDays; i++) {
      if (year === todayYear && month === todayMonth && i + 1 === todayDate) {
        calendar.push({
          label: i + 1,
          className: "actToday current-month",
  ...
  onArrowClick(increment) {
    switch (increment) {
      case true:
        ...
        } else if (this.checkIfYearIsWalkable(this.state.currentYear, true)) {
          this.incrementYear();
        }
        break;
      case false:
  ...
  monthPickerKeyDownHandler(e) {
    ...
    switch (e.keyCode) {
      case 39: // Right Arrow
        if (this.checkIfMonthIsWalkable(this.state.currentYear, index, true)) {
          if (index < 11) {
            ...
            let incButtonDisabled = !this.checkIfYearIsWalkable(
              this.state.currentYear + 1,
              true,
            );
            let decButtonDisabled = !this.checkIfYearIsWalkable(
              this.state.currentYear + 1,
              false,
            );
            this.setState({
      ...
      case 38: //Up Arrow
        if (index > 3) {
          if (
            this.checkIfMonthIsWalkable(this.state.currentYear, index - 4, 5)
          ) {
            ...
            if (index > 0) {
              ...
              if (date === 1) {
                if (month > 0) {
                  ...
                  this.setState({
                    setFocusTo:
                      this.amountOfDays(month - 1, year) +
                  ...
                  this.setState({
                    setFocusTo:
                      this.amountOfDays(11, year - 1) +
                      new Date(year - 1, 11, 1).getDay() -
                      1,
                  });
                }
              } else {
        ...
        case 38 /* Up Arrow */:
          {
            ...
            if (newDateDate < 1) {
              ...
              let newDateLastDayOfMonth = this.amountOfDays(
                newDateMonth,
                newDateYear,
              );
            ...
            if (
              this.props.minDate === null ||
              this.props.minDate === undefined ||
              new Date(newDateYear, newDateMonth, newDateDate) >=
                this.props.minDate
            ) {
              this.createCalendar(newDateYear, newDateMonth);
              let firstDayOfNewMonth = new Date(
                newDateYear,
                newDateMonth,
                1,
              ).getDay();
              this.setState({
                setFocusTo: newDateDate + firstDayOfNewMonth - 1,
              });
            }
          }
          e.preventDefault();
  ...
  checkIfMonthIsWalkable(currentYear, currentMonth, next) {
    ...
    if (
      (this.props.minDate === undefined &&
        new Date(year, month, 1) > this.props.maxDate) ||
      (this.props.maxDate === undefined &&
        new Date(year, month, monthLastDay) < this.props.minDate)
    ) {
      return false;
    } else return true;
  }
  ...
  checkIfYearIsWalkable(currentYear, next) {
    ...
    if (
      (this.props.minDate === undefined &&
        new Date(year, 0, 1) > this.props.maxDate) ||
      (this.props.maxDate === undefined &&
        new Date(year, 11, 31) < this.props.minDate)
    ) {
      return false;
    } else return true;
  }
  ...
  checkIfActToday(index) {
    ...
    if (
      this.actTodayDate === date.getDate() &&
      this.actTodayMonth === date.getMonth() &&
      this.actTodayYear === date.getFullYear()
    ) {
      return true;
    } else return false;
  }
  ...
  render() {
    ...
    return (
      <div
        ...
        className="date-picker position-wrapper"
      >
        <div
          ...
          <input
            ...
            value={
              this.props.value === null || this.props.value === undefined
                ? ""
                : ("0" + (this.props.value.getMonth() + 1)).slice(-2) +
                  "/" +
                  ("0" + this.props.value.getDate()).slice(-2) +
                  "/" +
        ...
        {this.state.isOpen && (
          <DatePickerDropDown
            ...
            performMultipleTimes={(callback) =>
              this.performMultipleTimes(callback)
            }
