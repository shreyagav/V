...
class Members extends Component {
  ...
  updateList(props) {
    ...
    Service.getFilteredMembers(actualFilters).then((json) => {
      this.setState({ members: json, loadData: false });
    });
  }
  ...
  renderFullNameColumn(value, row, index, col) {
    ...
    return (
      ...
      >
        <div
          className="flex-nowrap align-self-stretch align-center pr-05 pl-05"
        ...
        <Link to={"/member/" + row["id"]}>
          <span
            ...
            className="blue-link link"
          >
            <span style={{ fontSize: "1.1em", flex: "1 1 auto" }}>
              {row["firstName"] + " " + row["lastName"]}
            </span>
            <span style={{ flex: "1 1 auto" }} className="chapter">
              {row["siteName"]}
            </span>
          </span>
        </Link>
    ...
    const getNewValue = (value) => {
      if (value === null) {
        ...
        return (
          ...
          "/" +
          ("0" + dob.getDate().toString()).slice(-2) +
          "/" +
          dob.getFullYear()
        );
      }
    ...
    return (
      <div className="inner-pages-wrapper ipw-1000">
        ...
        <div className="flex-wrap align-center justify-space-between w-100 mb-05">
          ...
          <a className="big-static-button static-button" href="/new-member">
            <p>NEW MEMBER</p>
          </a>
        </div>
