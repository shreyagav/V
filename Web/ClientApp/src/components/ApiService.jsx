...
export class Service {
  ...
  static getCalendarEvents(month, year, sites) {
    return Service.__post(host + "/api/Calendar/GetMonthEvents", {
      month,
      year,
      sites,
    });
  ...
  static removeEventAttendee(id, attendee) {
    return Service.__post(
      host + "/api/Event/RemoveEventAttendees/" + id,
      attendee,
    );
  }
  ...
  static uploadPictures(eventId, input) {
    ...
    var promice = fetch(host + "/api/Event/UploadFile/" + eventId, {
      body: formData,
      method: "post",
    });
    promice.catch((err) => console.error(err));
